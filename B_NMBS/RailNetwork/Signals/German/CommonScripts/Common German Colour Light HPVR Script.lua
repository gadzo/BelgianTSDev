...
--------------------------------------------------------------------------------------
-- SET LIGHTS
-- Called by SetState to switch the appropriate Home lights for this signal type on/off according to its new state
...
--------------------------------------------------------------------------------------
-- SET VR LIGHTS
-- Called by SetState to switch the appropriate Distant lights for this signal type on/off according to its new state

function SetVRLights ( newState )
	
	if (newState == SIGNAL_CLEARED) then
		Call( "VR Signal Head:ActivateNode", "gr_mod_dist_Orange_1", 0 )
		Call( "VR Signal Head:ActivateNode", "gr_mod_dist_Green_1", 1 )
		Call( "VR Signal Head:ActivateNode", "gr_mod_dist_Orange_2", 0 )
		Call( "VR Signal Head:ActivateNode", "gr_mod_dist_Green_2", 1 )
	elseif (newState == SIGNAL_BLOCKED) then
		Call( "VR Signal Head:ActivateNode", "gr_mod_dist_Orange_1", 1 )
		Call( "VR Signal Head:ActivateNode", "gr_mod_dist_Green_1", 0 )
		Call( "VR Signal Head:ActivateNode", "gr_mod_dist_Orange_2", 1 )
		Call( "VR Signal Head:ActivateNode", "gr_mod_dist_Green_2", 0 )
	elseif (newState == SIGNAL_WARNING) then
		Call( "VR Signal Head:ActivateNode", "gr_mod_dist_Orange_1", 0 )
		Call( "VR Signal Head:ActivateNode", "gr_mod_dist_Green_1", 1 )
		Call( "VR Signal Head:ActivateNode", "gr_mod_dist_Orange_2", 1 )
	...
	elseif (newState == 0) then
		...
		Print( ("ERROR: SetVRLights trying to switch to invalid state " .. newState ) )
	end
end
...
function Update (time)
	...
	if gSignalState == SIGNAL_CLEARED then
		...
		-- And set VR lights too
		if gPrepared then
			SetVRLights( SIGNAL_CLEARED )
		else
			SetVRLights( 0 )
		end
	end
	...
	if (crossingStart == 1) then
		...
		if (prevFrontDist > 0 and prevBackDist > 0) then
			...
			if (linkIndex == 0) then
				...
				local consistMessageSent = false
				...
				-- If we've not stopped the train already based on home signal's state, check distant signal's state
				if not consistMessageSent then
				
					-- Send a warning message to the train if the next signal is blocked or at warning
					if (gLinkState[gConnectedLink] == SIGNAL_WARNING or gLinkState[gConnectedLink] == SIGNAL_WARNING2) then
...
function Occupied( linkIndex )
	...
	-- Set signal to BLOCKED
	SetState( SIGNAL_BLOCKED )
	
	-- Switch off VR signal
	SetVRLights(0)
...
function NotOccupied( linkIndex )
	...
	if (gConnectedLink == linkIndex) then
		...
		-- If we're prepared and not connected to a siding, set the VR lights to CLEAR
		if gPrepared and (linkIndex < gLinkCount - gYardEntryLinks) and not (linkIndex == 1 and gYardEntryLink1) then
			SetVRLights( SIGNAL_CLEARED )
			
		-- Otherwise turn the VR lights off
		else
			SetVRLights( 0 )
		end
	end
...
--------------------------------------------------------------------------------------
-- WARNING2 - HP signal is CLEAR but VR signal is at WARNING
--
function Warning2( linkIndex )

	DebugPrint( ( "DEBUG: Warning2(" .. linkIndex .. ")" ) )
	...
	if (gConnectedLink == linkIndex) then
		...
		-- If we're prepared and not connected to a siding, set the VR lights to WARNING
		if gPrepared and (linkIndex < gLinkCount - gYardEntryLinks) and not (linkIndex == 1 and gYardEntryLink1) then
			SetVRLights( SIGNAL_WARNING )
	...
	-- Set this link as Warning2 (regardless of whether we're going off line or not - this is the state of the link, NOT the signal!)
...
function Warning( linkIndex )
	...
	if (gConnectedLink == linkIndex) then
		...
		-- If we're prepared and not connected to a siding, set the VR lights to BLOCKED
		if gPrepared and (linkIndex < gLinkCount - gYardEntryLinks) and not (linkIndex == 1 and gYardEntryLink1) then
			SetVRLights( SIGNAL_BLOCKED )
...
function SignalPrepared( linkIndex )
	...
	if not gPrepared then
		...
		elseif gSignalState == SIGNAL_CLEARED then
			...
			newVRState = SIGNAL_CLEARED
			
		elseif gSignalState == SIGNAL_WARNING2 then
			newState = SIGNAL_CLEARED
			newVRState = SIGNAL_WARNING

		elseif gSignalState == SIGNAL_WARNING then
			newState = SIGNAL_WARNING
			newVRState = SIGNAL_BLOCKED
		end

		DebugPrint( ( "DEBUG: Link " .. linkIndex .. " is now Prepared" ) )
		gPrepared = true
		SetState( newState )
		SetVRLights( newVRState )
		gSignalState = newState
	end
end
...
function SignalReset( linkIndex )
	...
	if gPrepared then
		...
		-- Switch lights to red without changing signal state (SetState checks for signal being prepared)
		SetState( gSignalState )
		
		-- Switch off VR lights
		SetVRLights( 0 )
	end
end
